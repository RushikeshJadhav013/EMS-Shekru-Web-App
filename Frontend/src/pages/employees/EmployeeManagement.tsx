      ...
      try {
        ...
        const mappedData = data.map(toCamelCase).map((emp: any) => {
          ...
          if (emp.profilePhoto && !emp.profilePhoto.startsWith('http')) {
            emp.profilePhoto = `${import.meta.env.VITE_API_BASE_URL || 'http://172.105.56.142'}/${emp.profilePhoto}`;
          }
          if (emp.photoUrl && !emp.photoUrl.startsWith('http')) {
            emp.photoUrl = `${import.meta.env.VITE_API_BASE_URL || 'http://172.105.56.142'}/${emp.photoUrl}`;
          }
  ...
  const handleUpdateEmployee = async () => {
    ...
    try {
      ...
      if (mappedUpdated.profilePhoto && !mappedUpdated.profilePhoto.startsWith('http')) {
        mappedUpdated.profilePhoto = `${import.meta.env.VITE_API_BASE_URL || 'http://172.105.56.142'}/${mappedUpdated.profilePhoto}`;
      }
      if (mappedUpdated.photoUrl && !mappedUpdated.photoUrl.startsWith('http')) {
        mappedUpdated.photoUrl = `${import.meta.env.VITE_API_BASE_URL || 'http://172.105.56.142'}/${mappedUpdated.photoUrl}`;
      }
    ...
    for (const row of parsedRows) {
      ...
      try {
        ...
        if (mappedEmployee.profilePhoto && !mappedEmployee.profilePhoto.startsWith('http')) {
          mappedEmployee.profilePhoto = `http://172.105.56.142/${mappedEmployee.profilePhoto}`;
        }
        if (mappedEmployee.photoUrl && !mappedEmployee.photoUrl.startsWith('http')) {
          mappedEmployee.photoUrl = `http://172.105.56.142/${mappedEmployee.photoUrl}`;
        }
